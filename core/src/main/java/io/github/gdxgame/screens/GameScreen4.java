...
public class GameScreen4 implements Screen {
    ...
    private Texture backgroundTexture, pauseButtonTexture, slingshotTexture, redBirdTexture, blueBirdTexture, whiteBirdTexture, soundOnTexture, soundOffTexture;
    ...
    public GameScreen4(MyGame game) {
        this.game = game;
        this.currentLevel = 4;
    ...
    private void setupBirds() {
        ...
        for (int i = 0; i < 10; i++) {
            if (i % 3 == 0) {
                ...
                birdQueue.add(new Bird(whiteBirdTexture, 1.5f, 2, 40 , 40));
            } else if (i % 3 == 2) {
    ...
    private void reloadCurrentBird() {
        if (currentBird != null) {
            ...
            System.out.println("Cannot reload: No bird is currently loaded.");
        }
    }
    ...
    private void setupStructures() {
        ...
        for (int i = 0; i < 8; i++) {
            Block woodBase = new Block(woodTexture, baseX + i * blockWidth, baseY, blockWidth, blockHeight, 2, "wood");
            obstacles.add(woodBase);
            stage.addActor(woodBase);
        }

        for (int i = 0; i < 4; i++) {
            Block stoneBlock = new Block(stoneTexture, baseX + i * 2 * blockWidth, baseY + blockHeight, blockWidth, blockHeight, 3, "stone");
            obstacles.add(stoneBlock);
            stage.addActor(stoneBlock);

            Pig pig = new Pig(
                i % 2 == 0 ? kingPigTexture : queenPigTexture,
                baseX + (i * 2 + 1) * blockWidth + 5,
                baseY + blockHeight, 40, 40, 1,
                i % 2 == 0 ? "king" : "queen"
            );
            pigs.add(pig);
            stage.addActor(pig);
        ...
        for (int i = 0; i <= 3; i++) {
            Block glassColumn = new Block(
                glassTexture,
                baseX + i * 2 * blockWidth + 10,
                baseY + blockHeight * 2,
                blockWidth / 2,
                blockHeight * 4,
                1,
        ...
        obstacles.add(woodSupport);
        stage.addActor(woodSupport);

        for (int i = 0; i <= 3; i++) {
            Block woodTower = new Block(
                woodTexture,
                baseX + i * 2 * blockWidth + 10,
                baseY + blockHeight * 7,
                blockWidth / 2,
            ...
            obstacles.add(woodTower);
            stage.addActor(woodTower);
        }

        for (int i = 0; i < 2; i++) {
            Pig pig = new Pig(
                ministerPigTexture,
                baseX + (i * 4 + 2) * blockWidth + 10,
                baseY + blockHeight * 11,
                40, 40, 1,
                "minister"
            );
            pigs.add(pig);
